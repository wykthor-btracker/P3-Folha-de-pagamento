...
def main():
	...
	while(currCode.lower() != "e"):
		print(mainMenu)				
		currCode = raw_input(Output)
		if(currCode == "0"):
			new =  parseInput(raw_input("Informe nome, endereco, tipo, salario base separados por virgula: "),"empregado")
			if(new):
				payroll.add(new[0],new[1],new[2],int(new[3]))
			else:
				print("Entrada inválida, tente novamente")
		elif(currCode == "1"):
			rem = parseInput(raw_input("Informe o id do funcionário a ser removido: "),"id")
			if(rem):
				payroll.remove(rem)
			else:
				print("Entrada inválida, tente novamente.")
		elif(currCode == "2"):
			rem = parseInput(raw_input("Informe o id do empregado, o valor sem sifrao e a data da venda, separados por virgula: "),"salesCard")
			if(rem):
				emplo = payroll.find(rem[0])
				if(emplo):
					if(emplo.kind == "comissioned"):
						emplo.addSale(rem[1])
					else:
			...
			rem = parseInput(raw_input("Informe o id do funcionario para bater o ponto: "),id)
			if(rem):
				emplo = payroll.find(rem[0])
				if(emplo):
					if(emplo.kind == "hourly"):
						time = raw_input("Informe quantas horas foram trabalhadas hoje: ")
						emplo.punchOut(time)
					else:
			...
			rem = parseInput(raw_input("Informe a nova taxa do sindicado"),"tax")
			if(rem):
				syndicate.applyTax(float(rem[0]),int(rem[1]))
		elif(currCode == "5"):
			rem = parseInput(raw_input("Informe o id do funcionario a ser atualizado"),"id")
			if(rem):
				emplo = payroll.find(rem[0])
				if(emplo):
					new = raw_input("Nesta ordem, separados por vírgulas, informe os campos a serem atualizados, se for para manter o mesmo, ponha -1 no lugar:\nsalario,tipo,dia do pagamento,se faz parte do sindicato(s/n)")
